#!/usr/bin/env python3
"""
XLS-R + Conv1D + DPRNN + attention pooling ablation.

Replaces the BiLSTM in X-AWMD with a Dual-Path RNN (DPRNN) block.
This is the "DPRNN" row in the Temporal Head Variant section of Table V.

Usage:
    python ablation/xlsr_conv_dprnn.py --manifest /path/to/dataset_manifest.csv
"""
...
CFG = {
    ...
    "dprnn_layers": 2,
    "attn_dim": 128,
    "save_path": "./checkpoints/xlsr_conv_dprnn.pth",
...
class DPRNNBlock(nn.Module):
    def __init__(self, dim, hidden):
        super().__init__()
        self.intra = nn.LSTM(dim, hidden, batch_first=True, bidirectional=True)
        self.inter = nn.LSTM(dim, hidden, batch_first=True, bidirectional=True)
    ...
    def forward(self, x, chunk_size):
        ...
        x_intra = x.reshape(-1, K, C)
        y, _ = self.intra(x_intra)
        y = self.intra_fc(y).reshape(B, T2 // K, K, C)
        x = self.ln1(x + y)

        x_inter = x.permute(0, 2, 1, 3).contiguous().view(-1, T2 // K, C)
        y, _ = self.inter(x_inter)
        y = self.inter_fc(y).view(B, K, T2 // K, C).permute(0, 2, 1, 3)
        x = self.ln2(x + y)

        return x.view(B, T2, C)[:, :T, :]


class XLSRConvDPRNN(nn.Module):
    def __init__(self, cfg, in_dim):
        ...
        self.dprnn = nn.ModuleList([DPRNNBlock(c, cfg["dprnn_hidden"]) for _ in range(cfg["dprnn_layers"])])
    ...
    def forward(self, h):
        ...
        for blk in self.dprnn:
            x = blk(x, self.chunk_size)
        a = torch.softmax(self.attn(x).squeeze(-1), dim=1)
        return self.head((x * a.unsqueeze(-1)).sum(1)).squeeze(-1)


def main():
    ap = argparse.ArgumentParser(description="XLS-R + DPRNN ablation (Table V temporal head section)")
    ...
    ap.add_argument("--dprnn_hidden",        type=int,   default=None)
    ap.add_argument("--dprnn_layers",        type=int,   default=None)
    ...
    for k in ["manifest", "duration", "train_split", "val_split", "batch_size", "num_epochs", "lr",
              "xlsr_model", "layer_idx", "device", "save_path", "chunk_size", "dprnn_hidden",
              "dprnn_layers", "conv_layers", "conv_channels", "log_every", "early_stop_patience",
              "log_file", "seed"]:
    ...
    log(f"XLS-R (frozen) + DPRNN | layer={cfg['layer_idx']} | duration={cfg['duration']}s")
    ...
    model = XLSRConvDPRNN(cfg, in_dim=h.shape[-1]).to(device)
