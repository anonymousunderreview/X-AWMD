#!/usr/bin/env python3
"""
XLS-R + Conv1D + Transformer encoder + attention pooling ablation.

Replaces the BiLSTM in X-AWMD with a Transformer encoder.
This is the "Transformer" row in the Temporal Head Variant section of Table V.

Usage:
    python ablation/xlsr_conv_transformer.py --manifest /path/to/dataset_manifest.csv
"""
...
CFG = {
    ...
    "ff_dim": 512,
    "attn_dim": 128,
    "save_path": "./checkpoints/xlsr_conv_transformer.pth",
...
class XLSRConvTransformer(nn.Module):
    def __init__(self, cfg, in_dim):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=c, nhead=cfg["n_heads"], dim_feedforward=cfg["ff_dim"], batch_first=True)
...
def main():
    ap = argparse.ArgumentParser(description="XLS-R + Transformer ablation (Table V temporal head section)")
    ...
    for k in ["manifest", "duration", "train_split", "val_split", "batch_size", "num_epochs", "lr",
              "xlsr_model", "layer_idx", "device", "save_path", "log_every", "early_stop_patience",
              "log_file", "seed"]:
    ...
    log(f"XLS-R (frozen) + Transformer | layer={cfg['layer_idx']} | duration={cfg['duration']}s")
    ...
    model = XLSRConvTransformer(cfg, in_dim=h.shape[-1]).to(device)
